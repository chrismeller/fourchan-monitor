...
export class ThreadsController {
  ...
  async getThreadPages(
    ...
    for (const threadPage of response.data) {
      for (const thread of threadPage.threads) {
        ...
        const existing = this.threadsService.get(
          board,
          thread.no,
        );
  ...
  async checkThreadUpdate(
    ...
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.head(`https://a.4cdn.org/${board}/thread/${no}.json`, {
          headers: headers,
        }),
      );
      ...
      this.logger.error(`Unable to get ${board}: ${no} at HEAD request: ${e}`);
      return;
    }
  }
}
